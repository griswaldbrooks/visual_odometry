...
[[nodiscard]] auto benchmark_matcher(
    visual_odometry::image_matcher const& matcher, visual_odometry::image_loader& loader,
    std::optional<visual_odometry::camera_intrinsics> const& intrinsics) -> benchmark_result {
    ...
    while (loader.has_next()) {
        ...
        std::cout << "  Pair " << result.num_pairs << ": " << std::fixed << std::setprecision(2)
                  << duration_ms << " ms (" << num_matches << " matches";
        if (intrinsics) {
    ...
    if (result.num_pairs > 0) {
        ...
        result.avg_matches =
            static_cast<double>(result.total_matches) / static_cast<double>(result.num_pairs);
        if (intrinsics) {
            result.avg_inliers =
                static_cast<double>(result.total_inliers) / static_cast<double>(result.num_pairs);
            if (result.total_matches > 0) {
                result.avg_inlier_ratio = static_cast<double>(result.total_inliers) /
...
auto write_results_json(benchmark_result const& result, std::string_view output_path) -> void {
  ...
  "total_time_ms": )"
         << std::fixed << std::setprecision(2) << result.total_time_ms << R"(,
  "avg_time_ms": )"
         << std::fixed << std::setprecision(2) << result.avg_time_ms << R"(,
  "total_matches": )"
         << result.total_matches << R"(,
  "avg_matches": )"
         << std::fixed << std::setprecision(2) << result.avg_matches << R"(,
  "total_inliers": )"
         << result.total_inliers << R"(,
  "avg_inliers": )"
         << std::fixed << std::setprecision(2) << result.avg_inliers << R"(,
  "avg_inlier_ratio": )"
         << std::fixed << std::setprecision(4) << result.avg_inlier_ratio << R"(
}
)";
...
auto main(int argc, char* argv[]) -> int {  // NOLINT(bugprone-exception-escape)
    try {
        ...
        std::cout << "\nBenchmark Summary\n"
                  ...
                  << "  Avg time/pair: " << std::fixed << std::setprecision(2) << result.avg_time_ms
                  << " ms\n";
        if (intrinsics) {
